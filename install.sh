...
show_main_menu() {
    ...
    print_box_content "${RED}${BOLD}  [5]${RESET}  ${RED}🚪${RESET}  ${WHITE}${BOLD}Exit${RESET}             ${DIM}Close CarmonyOS Manager${RESET}" "$PURPLE"
    print_box_empty "$PURPLE"
...
show_config_preview() {
    ...
    print_box_content "   ${DIM}└── themes/ (Dynamic Island , CarmonyOS-Bar)${RESET}" "$GREEN"
...
create_desktop_file() {
    local py_file="$CONFIG_DIR/hypr/scripts/Settings.py"
    local desktop_dir="$HOME/.local/share/applications"
    local desktop_file="$desktop_dir/settings.desktop"
...
EOF
        ...
        print_success "Created: ${DIM}~/.local/share/applications/Settings.desktop${RESET}"
        print_success "CarmonyOS.py is now executable"
    else
        print_warning "Settings.py not found in configs"
    fi
}
...
show_exit_message() {
    ...
    print_gradient_line
    echo ""
    print_centered "CarmonyOS Manager v${VERSION}" "${DIM}${WHITE}"
    print_centered "${CODENAME}" "${DIM}${GRAY}"
    echo ""
    print_centered "github.com/ghvbb/CarmonyOS" "${DIM}${CYAN}"
    echo ""
    echo ""
}
...
show_about() {
    ...
    print_box "ℹ️  ABOUT CarmonyOS Manager" "$PURPLE"
    print_box_empty "$PURPLE"
